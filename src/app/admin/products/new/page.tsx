...
export default function NewProductPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    variants: [] as { sku: string; price: string; stock: number; options: { attribute: string; value: string }[] }[]
  });
    ...
    <div className="max-w-6xl mx-auto pb-20">
      ...
      <div className="flex items-center justify-between mb-10">
        <div>
          ...
          <p className="text-gray-500 font-semibold mt-2 text-lg">List a new premium item in your collection.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-10">
            ...
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <input required type="text" className="w-full px-5 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:bg-white focus:border-[#FF4D8D] outline-none transition-all font-bold text-gray-900" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="e.g. Brazilian Body Wave Frontal" />
                </div>
                <div className="space-y-3">
                  <label className="text-sm font-black text-gray-700 uppercase tracking-widest flex items-center gap-1.5">
                    Base Price ({currency})
                  </label>
                  <input required type="number" step="0.01" className="w-full px-5 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:bg-white focus:border-[#FF4D8D] outline-none transition-all font-bold text-gray-900" value={formData.basePrice} onChange={(e) => setFormData({...formData, basePrice: e.target.value})} placeholder="0.00" />
                </div>
              </div>

              <div className="space-y-3">
                <label className="text-sm font-black text-gray-700 uppercase tracking-widest">Description</label>
                <textarea required className="w-full px-5 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:bg-white focus:border-[#FF4D8D] outline-none transition-all font-bold text-gray-900 min-h-[160px]" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} placeholder="Describe the hair quality, source, and maintenance..." />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <label className="text-sm font-black text-gray-700 uppercase tracking-widest">Category</label>
                  <select required className="w-full px-5 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:bg-white focus:border-[#FF4D8D] outline-none transition-all font-bold text-gray-900 appearance-none" value={formData.categoryId} onChange={(e) => setFormData({...formData, categoryId: e.target.value})}>
                ...
                <div className="space-y-3">
                  ...
                  <input type="text" className="w-full px-5 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:bg-white focus:border-[#FF4D8D] outline-none transition-all font-bold text-gray-900" value={formData.tags} onChange={(e) => setFormData({...formData, tags: e.target.value})} placeholder="Best Seller, New Arrival" />
                </div>
              </div>
            </div>
          </section>
          ...
          {/* Variants Section */}
          <section className="bg-white p-6 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8 md:y-10">
                ...
                <div key={vIndex} className="p-5 md:p-8 border-2 border-gray-50 rounded-[1.5rem] md:rounded-[2rem] bg-gray-50/50 space-y-6 md:space-y-8 relative group/variant">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6">
                    <div className="space-y-2">
                      ...
                      <input type="text" placeholder="Auto-generated if empty" className="w-full px-4 py-3.5 md:py-3 rounded-xl bg-white border border-gray-200 focus:border-[#FF4D8D] outline-none transition-all font-bold text-sm placeholder:text-gray-300 placeholder:font-medium" value={v.sku} onChange={(e) => {
                        const newVariants = [...formData.variants];
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="number" step="0.01" placeholder="Leave blank for base" className="w-full px-4 py-3.5 md:py-3 rounded-xl bg-white border border-gray-200 focus:border-[#FF4D8D] outline-none transition-all font-bold text-sm" value={v.price} onChange={(e) => {
                        const newVariants = [...formData.variants];
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="number" className="w-full px-4 py-3.5 md:py-3 rounded-xl bg-white border border-gray-200 focus:border-[#FF4D8D] outline-none transition-all font-bold text-sm" value={v.stock} onChange={(e) => {
                        const newVariants = [...formData.variants];
                  ...
                  <div className="bg-white p-4 md:p-6 rounded-2xl border border-gray-100 space-y-4">
                    ...
                    <div className="space-y-4 md:space-y-3">
                      {v.options.map((opt, oIndex) => (
                        <div key={oIndex} className="flex flex-col sm:flex-row gap-3 sm:gap-2 items-stretch sm:items-center p-4 sm:p-0 bg-gray-50 sm:bg-transparent rounded-xl sm:rounded-none animate-in fade-in slide-in-from-top-2 duration-300">
                          <select className="px-4 py-3.5 sm:py-3 rounded-xl bg-white sm:bg-gray-50 border sm:border-transparent focus:border-[#FF4D8D] outline-none transition-all font-bold text-xs sm:w-1/3 appearance-none" value={opt.attribute} onChange={(e) => {
                            const newVariants = [...formData.variants];
                          ...
                          <div className="flex gap-2 flex-1 items-center">
                            <input type="text" placeholder="Value (e.g. 14 inches)" className="flex-1 px-4 py-3.5 sm:py-3 rounded-xl bg-white sm:bg-gray-50 border sm:border-transparent focus:border-[#FF4D8D] outline-none transition-all font-bold text-xs" value={opt.value} onChange={(e) => {
                              const newVariants = [...formData.variants];
